...
export function formatterSizeStr(size: number) {
  let obj = formatterSize(size);
  return obj.size + " " + obj.ext;
}
export function formatterSize(count: any): any {
  if (!count) {
...
export function formatBalanceStr(count: any) {
  if (!count) return "0";
...
export function formatAddressLong(addr: string) {
  if (!addr) return "";
...
export function toLocaleString(coin: any) {
  return coin = coin.toLocaleString("zh", { style: "decimal" });
}
...
export function formatPicUrl(fid: string, extend: string) {
